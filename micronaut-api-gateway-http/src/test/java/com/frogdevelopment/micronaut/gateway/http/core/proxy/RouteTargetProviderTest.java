...
class RouteTargetProviderTest {
    ...
    @Mock
    private AsyncLoadingCache<String, Optional<MatchingServiceEndpoint>> serviceEndpointCache;
    @Mock
    ...
    @BeforeEach()
    void beforeEach() {
        routeTargetProvider = new RouteTargetProvider(serviceEndpointCache, loadBalancerCache);
    }

    @Test
    void should_return_emptyMono_when_noServiceEndpointFound() {
        // given
        given(serviceEndpointCache.get("unknown-path")).willReturn(CompletableFuture.completedFuture(Optional.empty()));
    ...
    void should_return_emptyMono_when_noServiceInstanceFound() {
        // given
        given(serviceEndpointCache.get("my-path")).willReturn(CompletableFuture.completedFuture(Optional.of(matchingServiceEndpoint)));
