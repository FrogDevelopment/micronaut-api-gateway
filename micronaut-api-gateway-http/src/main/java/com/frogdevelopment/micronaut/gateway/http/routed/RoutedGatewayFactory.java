...
import com.frogdevelopment.micronaut.gateway.http.core.cache.ServiceEndpointCacheLoader;
    ...
    AsyncLoadingCache<String, Optional<MatchingServiceEndpoint>> serviceEndpointCache(
        ...
        final var pathMatchers = gatewayRouteProperties.getRoutes().entrySet()
                ...
                .flatMap(entry -> entry.getValue().getMapping()
                        .stream()
                        .<PathMatcher>map(subRoute -> new RoutesPathMatcher(entry.getKey(), subRoute.getRoute(), subRoute.getContext())))
                .toList();

        return Caffeine.newBuilder()
                .buildAsync(new ServiceEndpointCacheLoader(pathMatchers));
    }
